...
- `rowShift[i]` represents the number of positions to **cyclically left shift** the `i^th` row of `grid`.
- `colShift[j]` represents the number of positions to **cyclically upward shift** the `j^th` column of `grid`.
...
A **cyclic left shift** of the `i^th` row by `k` positions shifts only that row. The element at column `j` moves to column `(j - k + n) % n`, while all other rows remain unchanged.

A **cyclic upward shift** of the `j^th` column by `k` positions shifts only that column. The element at row `i` moves to row `(i - k + n) % n`, while all other columns remain unchanged.
